import mxnet as mx
import numpy as np
from net_block import clone_bn, clone_conv
...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        if normalization[k] > 0:
            ...
            scale = mx.symbol.Variable(name="{}_scale".format(from_name),
                shape=(1, num_channels.pop(0), 1, 1), init=mx.init.One())
    ...
    # testing python multibox_target
    loc_preds = mx.symbol.Concat(*loc_pred_layers, num_args=len(loc_pred_layers), dim=1)
    loc_preds = mx.symbol.reshape(data=loc_preds, shape=(0, -1, 4), name='multibox_loc_pred')
    cls_preds = mx.symbol.Concat(*cls_pred_layers, num_args=len(cls_pred_layers), dim=1)
    cls_preds = mx.symbol.reshape(data=cls_preds, shape=(0, -1, num_classes), name='multibox_cls_pred')
...
def multibox_layer_pvanet(from_layers, num_classes, is_test, sizes=[.2, .95], ratios=[1], clones=[], clip=True):
    """ multibox layer dedicated to pvanet_face """
    ...
    if len(clones) > 1:
        clone_ref = np.min(np.array(clones))
        clones = clones[1:]
    else:
        clone_ref = -1
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        # handle cloning
        if k in clones:
            # bn_pre = clone_bn(data=from_layer, bn_name='{}_pre_bn'.format(from_name), 
            #         src_layer=bn_pre_ref)
            relu_pre = mx.symbol.Activation(name='{}_pre_relu'.format(from_name), data=from_layer, act_type='relu')
            loc_pred_conv = clone_conv(data=relu_pre, conv_name='{}_loc_pred_conv'.format(from_name), 
                    src_layer=loc_pred_ref)
            cls_pred_conv = clone_conv(data=relu_pre, conv_name='{}_cls_pred_conv'.format(from_name), 
                    src_layer=cls_pred_ref)
        else:
            # append bn+relu 
            # bn_pre = mx.symbol.BatchNorm(name='{}_pre_bn'.format(from_name), data=from_layer, 
            #         use_global_stats=is_test)
            relu_pre = mx.symbol.Activation(name='{}_pre_relu'.format(from_name), data=from_layer, act_type='relu')
            loc_pred_conv = mx.symbol.Convolution(data=relu_pre, kernel=(3,3), \
                stride=(1,1), pad=(1,1), num_filter=num_loc_pred, \
                name="{}_loc_pred_conv".format(from_name))
            cls_pred_conv = mx.symbol.Convolution(data=relu_pre, kernel=(3,3), \
                stride=(1,1), pad=(1,1), num_filter=num_cls_pred, \
                name="{}_cls_pred_conv".format(from_name))
            if k == clone_ref:
                # bn_pre_ref = bn_pre
                loc_pred_ref = loc_pred_conv
                cls_pred_ref = cls_pred_conv

        # create location prediction layer
        loc_pred = mx.symbol.transpose(loc_pred_conv, axes=(0,2,3,1))
        # loc_pred = mx.symbol.reshape(data=loc_pred, shape=(0, -1, 4))
        loc_pred = mx.symbol.Flatten(data=loc_pred)
        loc_pred_layers.append(loc_pred)
        ...
        anchors = mx.symbol.MultiBoxPrior(from_layer, sizes=size_str, ratios=ratio_str, \
            clip=clip, name="{}_anchors".format(from_name))
        # anchors = mx.symbol.reshape(data=anchors, shape=(0, -1, 4))
        anchors = mx.symbol.Flatten(data=anchors)
        anchor_layers.append(anchors)
