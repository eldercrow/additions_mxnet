...
def inception_group(data,
    ...
    if num_filter_incep != n_curr_ch:
        ...
        syms['proj_data'] = s
    if use_crelu:
        concat_ = mx.sym.concat(concat_, -concat_)
        data = mx.sym.concat(data, -data)
        num_filter_incep *= 2
...
def clone_inception_group(data, prefix_group_name, src_syms):
    ...
    # else:
    #     data = incep_layers[0].get_children()[0].get_children() # the first bn layer
    return concat_ + data
...
def get_spotnet(n_classes, patch_size, use_global_stats, n_group=5):
    ...
    data = data / 128.0
    convu = mx.sym.Convolution(data, name='0/convx2', num_filter=12, kernel=(3,3), pad=(1,1))
    convu = subpixel_upsample(convu, 3, 2, 2)
    conv0 = bn_relu_conv(convu, prefix_name='0/', 
            num_filter=3, kernel=(3,3), pad=(1,1), stride=(1,1), use_crelu=True,
            use_global_stats=use_global_stats)
    pool0 = pool(conv0)
    concat0 = mx.sym.concat(pool0, data)
    conv1 = bn_relu_conv(concat0, prefix_name='1/', 
            num_filter=16, kernel=(3,3), pad=(1,1), stride=(1,1), use_crelu=True, 
            use_global_stats=use_global_stats)
    pool1 = pool(conv1)
    conv2 = bn_relu_conv(
        pool1,
