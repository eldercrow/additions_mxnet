...
def clone_inception_group(data, prefix_group_name, src_syms):
    ...
    for ii in range(3):
        ...
        conv_ = clone_bn_relu_conv(conv_, prefix_name, postfix_name,
                                   src_syms['unit{}'.format(ii + 1)])
        incep_layers.append(conv_)
...
def get_spotnet_xy(n_classes, patch_size, use_global_stats, n_group=5):
    ...
    for i in range(len(groups), n_group):
        ...
        for j in range(2):
            group_cloned = clone_inception_group(
                group_cloned, 'g{}/u{}'.format(i, j + 1), syms_unit[j])
