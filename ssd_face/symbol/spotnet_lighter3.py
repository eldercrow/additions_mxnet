...
def get_spotnet(n_classes, patch_size, use_global_stats, n_group=5):
    ...
    nf_proj = 32
    nf_upsamples = [[24, 32, 64], [32, 64], [64]]
    ctx_layers = []
    ...
    for i, g in enumerate(groups[:3]):
        ...
        hyper = relu_conv_bn(
            concat,
            prefix_name=hyper_names[i]+'1x1/',
            num_filter=nf_hyper_proj,
            kernel=(1, 1),
            pad=(0, 0),
            use_global_stats=use_global_stats)
        hyper = relu_conv_bn(
            hyper,
            prefix_name=hyper_names[i]+'3x3/',
            num_filter=nf_hyper,
            kernel=(3, 3),
            pad=(1, 1),
            use_global_stats=use_global_stats)
    ...
    conv096, src_syms1 = relu_conv_bn(
        groups[clone_ref],
        prefix_name='hyper096/1x1/',
        num_filter=nf_hyper_proj,
        kernel=(1, 1),
        pad=(0, 0),
        use_global_stats=use_global_stats,
        get_syms=True)
    conv096, src_syms3 = relu_conv_bn(
        conv096,
        prefix_name='hyper096/3x3/',
        num_filter=nf_hyper,
        kernel=(3, 3),
        pad=(1, 1),
        use_global_stats=use_global_stats,
    ...
    for i in range(clone_ref + 1, len(groups)):
        ...
        conv_ = clone_relu_conv_bn(
            groups[i], prefix_name=prefix_name+'1x1/', src_syms=src_syms1)
        conv_ = clone_relu_conv_bn(
            conv_, prefix_name=prefix_name+'3x3/', src_syms=src_syms3)
