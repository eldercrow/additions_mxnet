...
def multibox_layer(from_layers,
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        num_cls_pred = num_anchors * (num_classes + 2)
        ...
        if k == clone_ref:
            ...
            #         stride=(1, 1),
            #         pool_type='avg')
            #     fc_pools.append(pooli)
            # fc_as_conv = mx.sym.concat(*fc_pools)
            pred_conv, ref_syms = bn_relu_conv(
                fc_as_conv,
                prefix_name='{}_pred3/clone/'.format(from_name),
            ...
            pred_conv = bn_relu_conv(
                fc_as_conv,
                prefix_name='{}_pred3/'.format(from_name),
                num_filter=num_loc_pred + num_cls_pred,
                kernel=(3, 3),
                pad=(1, 1),
                num_group=1,
        ...
        pred_conv_bg = mx.sym.slice_axis(pred_conv, axis=2, begin=0, end=1)
        pred_conv_fg = mx.sym.slice_axis(pred_conv, axis=2, begin=1, end=4)
        pred_conv_fg = mx.sym.reshape(pred_conv_fg, shape=(0, 0, -4, -1, 1))
        pred_conv_bb = mx.sym.slice_axis(pred_conv, axis=2, begin=4, end=None)
        pred_conv_fg = mx.sym.Pooling(pred_conv_fg, kernel=(3, 1), pool_type='max')
        pred_conv_fg = mx.sym.reshape(pred_conv_fg, shape=(0, 0, 1))
        pred_conv = mx.sym.concat(pred_conv_bg, pred_conv_fg, pred_conv_bb, dim=2)
