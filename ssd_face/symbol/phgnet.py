...
def get_phgnet(n_classes, patch_size, use_global_stats, fix_gamma=False, n_group=5):
    ...
    syms_unit = []
    for j in range(2):
        group_i, n_curr_ch, syms = inception_group(group_i, 'g4/u{}'.format(j+1), n_curr_ch, 
    ...
    for i in range(len(groups), n_group):
        group_cloned = clone_bn_relu_conv(group_cloned, 'g{}/proj'.format(i+1), '', syms_proj)
        group_cloned = pool(group_cloned)
        for j in range(2):
            group_cloned = clone_inception_group(group_cloned, 'g{}/u{}'.format(i+1, j+1),  
                    syms_unit[j])
