...
def get_spotnet(n_classes, patch_size, use_global_stats, n_group=5):
    ...
    convu = mx.sym.Convolution(data / 128.0, name='0/convx2', num_filter=12, kernel=(3,3), pad=(1,1))
    convu = subpixel_upsample(convu, 3, 2, 2)
    concat0 = mx.sym.concat(convu, -convu)
    conv0 = bn_relu_conv(concat0, prefix_name='0/', 
            num_filter=6, kernel=(3,3), pad=(1,1), stride=(2,2), use_crelu=True,
            use_global_stats=use_global_stats)
    # pool0 = pool(conv0)
    conv1 = bn_relu_conv(conv0, prefix_name='1/', 
