    ...
    def _data_augmentation(self, data, label):
        ...
        if self.is_train and self._rand_samplers:
            ...
            for rs in self._rand_samplers:
                rand_crops += rs.sample(label, (data.shape[0], data.shape[1]))
            num_rand_crops = len(rand_crops)
            # randomly pick up one as input data
            if num_rand_crops > 0:
                index = np.random.choice(num_rand_crops, 1)[0] #int(np.random.uniform(0, 1) * num_rand_crops)
                width = data.shape[1]
                height = data.shape[0]
        ...
        if self.is_train:
            ...
            # cx = (label[valid_mask, 1] + label[valid_mask, 3]) / 2.0
        ...
        # for l in label:
        #     if l[0] == -1: continue
        #     x0 = int(np.maximum(0, l[1]))
        #     y0 = int(np.maximum(0, l[2]))
        #     x1 = int(np.minimum(width, l[3]))
        #     y1 = int(np.minimum(height, l[4]))
