    ...
    def sample(self, label, img_shape):
        ...
        for trial in range(self.max_trials):
            ...
            for i, bb in enumerate(gt):
                ...
                l = bb[0] if overlap > self.min_gt_overlap and new_size > self.min_gt_scale else -1
...
#         self.config = {'gt_constraint' : 'none'}
#
#     def sample(self, label, img_shape):
#         """
...
#             ground-truths
#
#         img_shape: (h, w), size of the image
#
#         Returns:
#         ----------
#         list of (crop_box, label) tuples, if failed, return empty list []
...
#             width = scale * ratio * min_sz
#             height = scale / ratio * min_sz
#             left = np.random.uniform(0., img_shape[1] - width)
#             top = np.random.uniform(0., img_shape[0] - height)
...
#                     continue
#                 new_gt_boxes = np.array(new_gt_boxes)
#                 label = np.tile(np.array((-1, -1, -1, -1, -1), dtype=np.float32), (label.shape[0], 1))
#                 label[:new_gt_boxes.shape[0], :] = new_gt_boxes
#                 rand_box = (l/img_shape[1], t/img_shape[0], r/img_shape[1], b/img_shape[0])
#                 # label = np.lib.pad(new_gt_boxes,
#                 #     ((0, label.shape[0]-new_gt_boxes.shape[0]), (0,0)), \
#                 #     'constant', constant_values=(-1, -1))
#                 samples.append((rand_box, label))
...
#         h[h < 0] = 0
#         inter_area = h * w
#         union_area = np.maximum(0, (rs-ls)*(bs-ts)) #np.ones(num_gt) * max(0, r - l) * max(0, b - t)
