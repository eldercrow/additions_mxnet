...
def multibox_layer(from_layers,
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        if k == clone_ref:
            ...
            #     use_global_stats=use_global_stats,
            #     get_syms=True)
            # dn_conv, sym_dn = data_norm(
            #         from_layer, name='{}_pred/clone/dn'.format(from_name), nch=128, get_syms=True)
            # relu_conv = mx.sym.Activation(dn_conv, act_type='relu')
            # pred_conv = mx.sym.Convolution(relu_conv, name='{}_pred/clone/conv'.format(from_name), 
            #         num_filter=num_cls_pred+num_loc_pred, kernel=(3,3), pad=(1,1), no_bias=False)
            # sym_conv = pred_conv
            #
            ...
            #     src_syms=ref_fc)
            # dn_conv = data_norm(
            #         from_layer, name='{}_pred/clone/dn'.format(from_name), nch=128, bias=sym_dn['bias'])
            # relu_conv = mx.sym.Activation(dn_conv, act_type='relu')
            # pred_conv = clone_conv(relu_conv, name='{}_pred/clone/conv'.format(from_name), src_layer=sym_conv)
            #
            ...
            #     use_global_stats=use_global_stats)
            # dn_conv = data_norm(
            #         from_layer, name='{}_pred/dn'.format(from_name), nch=128, get_syms=False)
            # relu_conv = mx.sym.Activation(dn_conv, act_type='relu')
            # pred_conv = mx.sym.Convolution(relu_conv, name='{}_pred/conv'.format(from_name), 
            #         num_filter=num_cls_pred+num_loc_pred, kernel=(3,3), pad=(1,1), no_bias=False)
            #
