...
def parse_args():
    ...
    parser.add_argument(
        '--dataset',
        dest='dataset',
        help='which dataset to use, check .dataset directory',
        default='wider',
        type=str)
    parser.add_argument(
        '--image-set',
        dest='image_set',
        help='train set, can be trainval or train',
        default='trainval',
        type=str)
    ...
    parser.add_argument(
        ...
        dest='devkit_path',
        help='VOCdevkit path',
        default=os.path.join(os.getcwd(), 'data', 'VOCdevkit'),
        type=str)
    parser.add_argument(
        '--network',
        dest='network',
    ...
    parser.add_argument(
        '--gpus',
        dest='gpus',
        help='GPU devices to train with',
        default='0',
        type=str)
    parser.add_argument(
        '--cpu',
        dest='use_cpu',
        help='Use CPU, overrides --gpus option',
        default=0,
        type=int)
    parser.add_argument(
        '--begin-epoch',
...
if __name__ == '__main__':
    ...
    train_net(args.network, args.dataset, args.image_set, args.devkit_path,
              args.batch_size, args.data_shape, [
                  args.mean_r, args.mean_g, args.mean_b
              ], args.resume, args.finetune, args.pretrained,
              args.from_scratch, args.epoch, args.prefix, ctx,
