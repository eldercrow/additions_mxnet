...
def prepare_groups(group_i, use_global_stats):
    ...
    ctx_group = prepare_context(groups[0], use_global_stats)
    for i in range(3):
        groups[i] = mx.sym.concat(groups[i], ctx_group[i])
...
def prepare_context(data, use_global_stats):
    ...
    for i, nf in enumerate(nf_ctx, 1):
        ctx = relu_conv_bn(ctx, 'ctxproj{}/1x1/'.format(i),
                num_filter=nf, kernel=(1, 1), pad=(0, 0),
                use_global_stats=use_global_stats)
        ctx = relu_conv_bn(ctx, 'ctxproj{}/3x3/'.format(i),
