...
def get_config(network, data_shape, **kwargs):
    ...
    if network == 'vgg16_reduced':
        ...
        python_anchor = True
        return locals()
    elif network in ('dilatenetv1',):
        from_layers = [('hyper{}/1'.format(i), 'hyper{}/2'.format(i)) for i in range(6)]
        num_filters = [-1] * 6
        strides = [-1] * 6
        pads = [-1] * 6
        ratios = [[1.0, 2.0, 0.5] for _ in from_layers]
        sizes = [[72, 48], [144, 96], [288, 192], \
                 [576, 384], [data_shape-144, data_shape-96], [data_shape-48, data_shape]]
        sizes = np.array(sizes) / float(data_shape)
        sizes = sizes.tolist()
        sizes[-1] = [sizes[-1][0],]
        normalizations = -1
...
=======
    elif network in ('dilatenetv5', 'dilatenetv6'):
        from_layers = [('hyper{}/1'.format(i), 'hyper{}/2'.format(i)) for i in range(6)]
        num_filters = [-1] * 6
        strides = [-1] * 6
        pads = [-1] * 6
        ...
        python_anchor = True
        return locals()
    elif network in ('dilatenetv4', 'hypernetv4', 'dilatenetv3'):
