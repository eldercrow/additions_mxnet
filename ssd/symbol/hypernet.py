...
def prepare_groups(group_i, n_curr_ch, use_global_stats):
    ...
    # 48 24 12 6 3
    nf_3x3 = [(128, 256), (256, 512), (384, 768), (256, 512), (256,), (256,)]
    nf_1x1 = [64, 128, 192, 128, 128, 256]
    n_unit = [2, 3, 2, 1, 1, 1]

    nf_dn = [0, 0, 0, 128, 128, 128]
    nf_up = [192, 128]
    ...
    for i, (nf3, nf1) in enumerate(zip(nf_3x3, nf_1x1)):
        ...
        group_i = bn_relu_conv(group_i, 'gp{}/'.format(i), num_filter=nf3[-1],
                kernel=(1, 1), pad=(0, 0),
                use_global_stats=use_global_stats)
    ...
    for i, g in enumerate(groups[1:3]):
        up_groups[i].append( \
                upsample_feature(g, name='up{}{}/'.format(i+1, i), scale=2,
                    num_filter_proj=nf_up[i], num_filter_upsample=0, use_global_stats=use_global_stats))
    up_groups[0].append( \
            upsample_feature(groups[2], name='up20/', scale=4,
                num_filter_proj=nf_up[0], num_filter_upsample=0, use_global_stats=use_global_stats))
...
def get_symbol(num_classes=1000, **kwargs):
    ...
    bn3_1 = multiple_conv(pool2, '3/1/',
            num_filter_3x3=(64, 128), num_filter_1x1=32,
            use_global_stats=use_global_stats)
    bn3_2 = multiple_conv(bn3_1, '3/2/',
            num_filter_3x3=(64, 128), num_filter_1x1=32,
            use_global_stats=use_global_stats)
    bn3 = bn3_1 + bn3_2
    ...
    for i, (g, u, d) in enumerate(zip(groups, up_groups, dn_groups)):
        ...
        h1 = bn_relu_conv(p, 'hyperc{}/1/'.format(i),
                ...
                use_global_stats=use_global_stats)
        h1 = mx.sym.BatchNorm(h1, name='hyper{}/1/bn'.format(i),
        ...
        h2 = bn_relu_conv(p, 'hyperc{}/2/'.format(i),
                num_filter=nf_hyper[i], kernel=(1, 1), pad=(0, 0),
                use_global_stats=use_global_stats)
        h2 = mx.sym.BatchNorm(h2, name='hyper{}/2/bn'.format(i),
                use_global_stats=use_global_stats, fix_gamma=False)
