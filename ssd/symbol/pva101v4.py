...
def prepare_context(data, use_global_stats):
    ...
    for i, (nf3, nf1, dil) in enumerate(zip(nf_3x3, nf_1x1, dilate)):
        for j in range(n_unit[i]):
            ctx = conv_bn_relu(ctx, 'ctxdil{}/1x1/u{}'.format(i, j),
                    num_filter=nf1, kernel=(1, 1), pad=(0, 0),
                    use_global_stats=use_global_stats)
            ctx = conv_bn_relu(ctx, 'ctxdil{}/3x3/u{}'.format(i, j),
    ...
    for i, nf in enumerate(nf_ctx, 1):
        ctx = conv_bn_relu(ctx, 'ctx{}/1x1'.format(i),
                num_filter=nf, kernel=(1, 1), pad=(0, 0),
                use_global_stats=use_global_stats)
        ctx = conv_bn_relu(ctx, 'ctx{}/3x3'.format(i),
...
def pvanet_preact(data, use_global_stats=True, no_bias=False):
    ...
    ctx_all = prepare_context(conv3, use_global_stats)
    for i, ctx in enumerate(ctx_all):
        groups[i] = mx.sym.concat(groups[i], ctx)

    nf_group = [384, 512, 512, 384, 384, 256]
    for i, (g, nf) in enumerate(zip(groups, nf_group)):
        g = conv_bn_relu(g, 'gp1x1/{}'.format(i), num_filter=nf/2,
                kernel=(1, 1), pad=(0, 0), use_global_stats=use_global_stats)
        groups[i] = conv_bn_relu(g, 'gp3x3/{}'.format(i), num_filter=nf,
                kernel=(3, 3), pad=(1, 1), use_global_stats=use_global_stats)

    hyper_group = []
    nf_hyper = [256, 256, 256, 192, 192, 128]
