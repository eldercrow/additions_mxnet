...
from common import convolution, fullyconnected, batchnorm, multibox_layer_python, subpixel_upsample
...
def pvanet_multibox(data, num_classes, patch_size=512, use_global_stats=True, no_bias=False, lr_mult=1.0):
    ...
    projf = conv_bn_relu(concat, group_name='projf_16',
            ...
            use_global_stats=use_global_stats, lr_mult=lr_mult)
    # 24px, upsample hyperfeature
    upconv = conv_bn_relu(projf, group_name='conv_08',
            num_filter=128, pad=(1,1), kernel=(3,3), stride=(1,1), no_bias=True,
            use_global_stats=use_global_stats, lr_mult=lr_mult)
    upconv = subpixel_upsample(upconv, 32, 2, 2)
    concatf8 = mx.sym.concat(conv3, upconv)
    projf8 = conv_bn_relu(concatf8, group_name='projf_08',
            num_filter=64, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=True,
            use_global_stats=use_global_stats, lr_mult=lr_mult)
    convf8 = conv_bn_relu(projf8, group_name='convf_08',
    ...
    from_layers = [convf8, convf]
    sz_ratio = power(2.0, 1.0 / 4.0)
    sizes = [[24.0 * sz_ratio, 24.0 / sz_ratio], [48.0 * sz_ratio, 48.0 / sz_ratio]]
    feat_strides = [8, 16, 32, 64, 128, 256, 512]
