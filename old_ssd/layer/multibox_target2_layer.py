...
class MultiBoxTarget2(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        # max_probs_cls = mx.nd.slice_axis(in_data[4], axis=1, begin=0, end=1).asnumpy()
        # max_probs_cls = 1 - np.reshape(max_probs_cls, (n_batch, -1))
        # probs_cls = in_data[4].asnumpy()
        max_cids = mx.nd.argmax(in_data[4], axis=1).asnumpy().astype(int)
        probs_bg_cls = mx.nd.slice_axis(in_data[4], axis=1, begin=0, end=1).asnumpy()
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        sample_cls = out_data[2].asnumpy().astype(int)
        ...
        for i, tloc in enumerate(target_locs):
            ...
            if bid == -1 or aid == -1:
                continue
            sf = 1.0 / self.hard_neg_ratio if sample_cls[i] == 0 else 1 
...
class MultiBoxTargetProp2(mx.operator.CustomOpProp):
    def __init__(self, n_class, img_wh,
            th_iou=0.5, th_iou_neg=1.0/3.0, th_nms_neg=1.0/3.0, max_pos_sample=5120,
            reg_sample_ratio=1.0, hard_neg_ratio=3.0, ignore_label=-1, # box_ratios=(1.0,),
