...
class RandScaler(RandSampler):
    ...
    def sample(self, label, img_shape):
        ...
        gt[:, 2::2] *= img_shape[0]
        samples = []
        base_scale = self.patch_size / float(np.minimum(img_shape[0], img_shape[1]))
        # base_scale = self.patch_size / float(np.sqrt(img_shape[0] * img_shape[1]))
        for trial in range(self.max_trials):
            scale = base_scale * (1.0 + np.random.uniform(-self.scale_exp, self.scale_exp))
            # scale = base_scale * np.power(2.0, np.random.uniform(-self.scale_exp, self.scale_exp))
            asp = np.sqrt(1.0 + np.random.uniform(-0.05, 0.05))
            scale_x = scale * asp
            scale_y = scale / asp
