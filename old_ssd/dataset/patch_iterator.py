...
class PatchIter(mx.io.DataIter):
    ...
    1. Load an image.
    2. For each face, crop a patch centered by the face (256 by 256 for example).
    ...
    def _get_batch(self):
        ...
        for i, (pidx, l) in enumerate(zip(patch2img, batch_label)):
            patch = self._crop_patch(batch_imgs[pidx], l[6:].astype(int), l)
            batch_data[i] = patch
    ...
    def _crop_patch(self, img, roi, label):
        hh = img.shape[0]
        ww = img.shape[1]
        ...
        # import ipdb
        # plt.imshow(patch.asnumpy() / 255.0)
        # print int(label[1])
        # print label[2:6] * self._data_shape[1]
        # ipdb.set_trace()
