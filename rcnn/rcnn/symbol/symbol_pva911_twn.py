...
def bn_relu_conv(data, group_name, shape, pad, stride, use_global=True, use_crelu=False, get_syms=False):
    """ used in mCReLU """
    bn_name = group_name + '/bn'
    relu_name = group_name + '/relu'
    conv_name = group_name + '/conv'
...
def mCReLU(data, group_name, filters, strides, use_global, n_curr_ch):
    ...
    for i in range(3):
        conv, s = bn_relu_conv(data=conv, group_name=group_name+'/{}'.format(i+1), 
                shape=shapes[i], pad=pads[i], stride=strides[i], 
    ...
    if need_proj:
        proj = conv_twn(data=ssyms[0]['relu'], name=group_name+'/proj',
                shape=(filters[2], n_curr_ch, 1, 1), pad=(0,0), stride=(ss,ss))
...
def inception(data, group_name, 
    ...
    incep_0 = conv_bn_relu(data=incep_relu, group_name=group_name_0, 
            shape=(filter_0, n_curr_ch, 1, 1), pad=(0,0), stride=stride, use_global=use_global)

    incep_1_reduce = conv_bn_relu(data=incep_relu, group_name=group_name_1+'_reduce', 
            shape=(filters_1[0], n_curr_ch, 1, 1), pad=(0,0), stride=stride, use_global=use_global)
    incep_1_0 = conv_bn_relu(data=incep_1_reduce, group_name=group_name_1+'_0', 
            shape=(filters_1[1], filters_1[0], 3, 3), pad=(1,1), stride=(1,1), use_global=use_global)

    incep_2_reduce = conv_bn_relu(data=incep_relu, group_name=group_name_2+'_reduce', 
            shape=(filters_2[0], n_curr_ch, 1, 1), pad=(0,0), stride=stride, use_global=use_global)
    incep_2_0 = conv_bn_relu(data=incep_2_reduce, group_name=group_name_2+'_0', 
            shape=(filters_2[1], filters_2[0], 3, 3), pad=(1,1), stride=(1,1), use_global=use_global)
    incep_2_1 = conv_bn_relu(data=incep_2_0, group_name=group_name_2+'_1', 
            shape=(filters_2[2], filters_2[1], 3, 3), pad=(1,1), stride=(1,1), use_global=use_global)
    # incep_0 = conv_bn_relu(data=incep_relu, group_name=group_name_0, 
    ...
    # incep_2_1 = conv_bn_relu(data=incep_2_0, group_name=group_name_2+'_1', 
    #         num_filter=filters_2[2], kernel=(3,3), pad=(1,1), stride=(1,1), use_global=use_global)
    ...
    if filter_p is not None:
        incep_p_pool = mx.sym.Pooling(name=incep_name+'/pool', data=incep_relu, pooling_convention='full', 
                pad=(0,0), kernel=(3,3), stride=(2,2), pool_type='max')
        incep_p_proj = conv_bn_relu(data=incep_p_pool, group_name=incep_name+'/poolproj', 
    ...
    out_conv = conv_twn(data=incep, name=group_name+'/out/conv', 
            shape=(filter_out, nch_incep, 1, 1), stride=(1,1), pad=(0,0), no_bias=True)
    # final_bn is to handle stupid redundancy in the original model
    if final_bn:
        out_conv = mx.sym.BatchNorm(name=group_name+'/out/bn', data=out_conv, 
                use_global_stats=use_global, fix_gamma=False, eps=1e-05)
    
    if n_curr_ch != filter_out or stride[0] > 1:
        out_proj = conv_twn(name=group_name+'/proj', data=data, 
...
def pvanet_preact(data, is_test):
    """ PVANet 9.0 """
    conv1_1_conv = mx.sym.Convolution(name='conv1_1/conv', data=data, 
            num_filter=16, pad=(3,3), kernel=(7,7), stride=(2,2), no_bias=True)
    conv1_1_concat = mx.sym.Concat(name='conv1_1/concat', *[conv1_1_conv, -conv1_1_conv])
    conv1_1_bn = mx.sym.BatchNorm(name='conv1_1/bn', data=conv1_1_concat, 
            use_global_stats=is_test, fix_gamma=False, eps=1e-05)
    conv1_1_relu = mx.sym.Activation(name='conv1_1/relu', data=conv1_1_bn, act_type='relu')
    pool1 = mx.sym.Pooling(name='pool1', data=conv1_1_relu, 
            pooling_convention='full', pad=(0,0), kernel=(3,3), stride=(2,2), pool_type='max')
    nch_pool1 = 32
    
    # no pre bn-scale-relu for 2_1_1
    conv2_1_1_conv = conv_twn(name='conv2_1/1/conv', data=pool1, 
            shape=(24, 32, 1, 1), pad=(0,0), no_bias=False)
    conv2_1_2_conv = bn_relu_conv(data=conv2_1_1_conv, group_name='conv2_1/2', 
            shape=(24, 24, 3, 3), pad=(1,1), stride=(1,1), use_global=is_test)
    conv2_1_3_conv = bn_relu_conv(data=conv2_1_2_conv, group_name='conv2_1/3', 
            shape=(64, 48, 1, 1), pad=(0,0), stride=(1,1), use_global=is_test, use_crelu=True)
    conv2_1_proj = conv_twn(name='conv2_1/proj', data=pool1, 
    ...
    conv3_1, n_curr_ch, conv3_1_proj = mCReLU(data=conv2_3_ror, group_name='conv3_1', 
    ...
    conv4_1, n_curr_ch, conv4_1_proj = inception(data=conv3_4_ror, group_name='conv4_1', 
    ...
    conv4_2, n_curr_ch = inception(data=conv4_1, group_name='conv4_2', 
            filter_0=64, filters_1=(64,128), filters_2=(24,48,48), filter_p=None, filter_out=256, 
    ...
    conv5_1, n_curr_ch, conv5_1_proj = inception(data=conv4_4_ror, group_name='conv5_1', 
            filter_0=64, filters_1=(96,192), filters_2=(32,64,64), filter_p=128, filter_out=384, 
            stride=(2,2), use_global=is_test, n_curr_ch=n_curr_ch)
    conv5_2, n_curr_ch = inception(data=conv5_1, group_name='conv5_2', 
            filter_0=64, filters_1=(96,192), filters_2=(32,64,64), filter_p=None, filter_out=384, 
            stride=(1,1), use_global=is_test, n_curr_ch=n_curr_ch)
    ...
    conv2_1_long_proj = conv_twn(pool1, name='conv2_1/long/proj', 
            shape=(384, nch_pool1, 1, 1), stride=(8,8))
    conv5_4_long_ror = conv5_4_ror + conv2_1_long_proj

    # final layers
    conv5_4_last_bn = mx.sym.BatchNorm(conv5_4_long_ror, name='conv5_4/last_bn', 
            use_global_stats=is_test, eps=1e-05, fix_gamma=False)
    conv5_4_last_relu = mx.sym.Activation(conv5_4_last_bn, name='conv5_4/last_relu', act_type='relu')

    # hyperfeature
    downsample = mx.sym.Pooling(name='downsample', data=conv3_4_ror, pooling_convention='full', 
            kernel=(3,3), stride=(2,2), pool_type='max')
    upsample = mx.sym.UpSampling(name='upsample', data=conv5_4_last_relu, scale=2, 
    ...
    nch_concat = nch_3_4 + nch_4_4 + nch_5_4
    convf_rpn = conv_twn(name='convf_rpn', data=concat, 
            shape=(128, nch_concat, 1, 1), pad=(0,0), stride=(1,1))
    reluf_rpn = mx.sym.Activation(name='reluf_rpn', data=convf_rpn, act_type='relu')

    convf_2 = conv_twn(name='convf_2', data=concat, 
            shape=(384, nch_concat, 1, 1), pad=(0,0), stride=(1,1))
...
def get_pvanet_twn_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    # RPN layers
    rpn_conv1 = conv_twn(name='rpn_conv1', data=reluf_rpn, 
            shape=(384, 128, 3, 3), pad=(1,1), stride=(1,1))
    rpn_relu1 = mx.sym.Activation(name='rpn_relu1', data=rpn_conv1, act_type='relu')
    rpn_cls_score = mx.sym.Convolution(name='rpn_cls_score', data=rpn_relu1, 
            num_filter=2*num_anchors, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=False)
    rpn_bbox_pred = mx.sym.Convolution(name='rpn_bbox_pred', data=rpn_relu1, 
    ...
    # Fast R-CNN
    roi_pool = mx.sym.ROIPooling(name='roi_pool5', data=concat_convf, rois=rois, 
            pooled_size=(6, 6), spatial_scale=1.0 / config.RCNN_FEAT_STRIDE)
    flat5 = mx.sym.Flatten(name='flat5', data=roi_pool)
    # fc6 = fc_twn(name='fc6', data=flat5, shape=(4096, 6*6*512), no_bias=False)
    fc6 = mx.sym.FullyConnected(name='fc6', data=flat5, num_hidden=4096, no_bias=False)
    fc6_bn = mx.sym.BatchNorm(name='fc6/bn', data=fc6, use_global_stats=True, fix_gamma=False, eps=1e-05)
    fc6_dropout = mx.sym.Dropout(name='fc6/dropout', data=fc6_bn, p=0.25)
    fc6_relu = mx.sym.Activation(name='fc6/relu', data=fc6_dropout, act_type='relu')
    # fc7 = fc_twn(name='fc7', data=fc6_relu, shape=(4096, 4096), no_bias=False)
    fc7 = mx.sym.FullyConnected(name='fc7', data=fc6_relu, num_hidden=4096, no_bias=False)
    fc7_bn = mx.sym.BatchNorm(name='fc7/bn', data=fc7, use_global_stats=True, fix_gamma=False, eps=1e-05)
    fc7_dropout = mx.sym.Dropout(name='fc7/dropout', data=fc7_bn, p=0.25)
    fc7_relu = mx.sym.Activation(name='fc7/relu', data=fc7_dropout, act_type='relu')

    # classification
    cls_score = mx.sym.FullyConnected(name='cls_score', data=fc7_relu, num_hidden=num_classes)
    cls_prob = mx.sym.SoftmaxOutput(name='cls_prob', data=cls_score, label=label, normalization='batch')
    # bounding box regression
    bbox_pred = mx.sym.FullyConnected(name='bbox_pred', data=fc7_relu, num_hidden=num_classes*4)
    bbox_loss_ = bbox_weight * \
            mx.sym.smooth_l1(name='bbox_loss_', scalar=1.0, data=(bbox_pred - bbox_target))
    bbox_loss = mx.sym.MakeLoss(name='bbox_loss', data=bbox_loss_, grad_scale=1.0 / config.TRAIN.BATCH_ROIS)

    # reshape output
    label = mx.sym.Reshape(name='label_reshape', data=label, shape=(config.TRAIN.BATCH_IMAGES, -1))
    cls_prob = mx.sym.Reshape(name='cls_prob_reshape', data=cls_prob, 
            shape=(config.TRAIN.BATCH_IMAGES, -1, num_classes))
    bbox_loss = mx.sym.Reshape(name='bbox_loss_reshape', data=bbox_loss, 
...
def get_pvanet_twn_test(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    bbox_pred = mx.sym.Reshape(name='bbox_pred_reshape', data=bbox_pred, 
