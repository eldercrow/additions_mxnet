...
def conv_bn_relu(data, group_name, num_filter, kernel, pad, stride, use_global=True):
    """ used in mCReLU """
    bn_name = group_name + '/bn'
    relu_name = group_name + '/relu'
    conv_name = group_name + '/conv'

    conv = mx.sym.Convolution(name=conv_name, data=data, 
            num_filter=num_filter, pad=pad, kernel=kernel, stride=stride, no_bias=True, cudnn_off=False)
    bn = mx.sym.BatchNorm(name=bn_name, data=conv, use_global_stats=use_global, fix_gamma=False, eps=1e-05)
...
def inception(data, group_name, 
    ...
    # final_bn is to handle stupid redundancy in the original model
    if final_bn:
        out_conv = mx.sym.Convolution(name=group_name+'/out/conv', data=incep, 
                num_filter=filter_out, kernel=(1,1), stride=(1,1), pad=(0,0), no_bias=True, cudnn_off=False)
        out_conv = mx.sym.BatchNorm(name=group_name+'/out/bn', data=out_conv, 
                use_global_stats=use_global, fix_gamma=False, eps=1e-05)
    else:
        out_conv = mx.sym.Convolution(name=group_name+'/out/conv', data=incep, 
                num_filter=filter_out, kernel=(1,1), stride=(1,1), pad=(0,0), cudnn_off=False)
    
    if n_curr_ch != filter_out or stride[0] > 1:
        out_proj = mx.sym.Convolution(name=group_name+'/proj', data=data, 
                num_filter=filter_out, kernel=(1,1), stride=stride, pad=(0,0), cudnn_off=False)
...
def pvanet_preact(data, is_test):
    ...
    conv1_1_conv = mx.sym.Convolution(name='conv1_1/conv', data=data, 
            num_filter=16, pad=(3,3), kernel=(7,7), stride=(2,2), no_bias=True, cudnn_off=False)
    ...
    conv2_1_1_conv = mx.sym.Convolution(name='conv2_1/1/conv', data=pool1, 
            num_filter=24, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=False, cudnn_off=False)
    ...
    conv2_1_3_conv = bn_relu_conv(data=conv2_1_2_conv, group_name='conv2_1/3', 
            num_filter=64, kernel=(1,1), pad=(0,0), stride=(1,1), use_global=is_test, use_crelu=True)
    conv2_1_proj = mx.sym.Convolution(name='conv2_1/proj', data=pool1, 
            num_filter=64, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=False, cudnn_off=False)
    ...
    # final layers
    conv5_4_last_bn = mx.sym.BatchNorm(name='conv5_4/last_bn', data=conv5_4, 
            use_global_stats=is_test, eps=1e-05, fix_gamma=False)
    conv5_4_last_relu = mx.sym.Activation(name='conv5_4/last_relu', data=conv5_4_last_bn, act_type='relu')

    # hyperfeature
    downsample = mx.sym.Pooling(name='downsample', data=conv3_4, pooling_convention='full', 
    ...
    concat = mx.sym.Concat(name='concat', *[downsample, conv4_4, upsample])
    convf_rpn = mx.sym.Convolution(name='convf_rpn', data=concat, 
            num_filter=128, pad=(0,0), kernel=(1,1), stride=(1,1), cudnn_off=False)
    reluf_rpn = mx.sym.Activation(name='reluf_rpn', data=convf_rpn, act_type='relu')

    convf_2 = mx.sym.Convolution(name='convf_2', data=concat, 
            num_filter=384, pad=(0,0), kernel=(1,1), stride=(1,1), cudnn_off=False)
...
def get_pvanet_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    rpn_conv1 = mx.sym.Convolution(name='rpn_conv1', data=reluf_rpn, 
            num_filter=384, pad=(1,1), kernel=(3,3), stride=(1,1), cudnn_off=False)
    ...
    rpn_cls_score = mx.sym.Convolution(name='rpn_cls_score', data=rpn_relu1, 
            num_filter=2*num_anchors, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=False, cudnn_off=False)
    rpn_bbox_pred = mx.sym.Convolution(name='rpn_bbox_pred', data=rpn_relu1, 
            num_filter=4*num_anchors, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=False, cudnn_off=False)
...
def get_pvanet_test(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    # RPN layers
    rpn_conv1 = mx.sym.Convolution(name='rpn_conv1', data=reluf_rpn, 
            num_filter=384, pad=(1,1), kernel=(3,3), stride=(1,1), no_bias=False, cudnn_off=False)
    rpn_relu1 = mx.sym.Activation(name='rpn_relu1', data=rpn_conv1, act_type='relu')
    rpn_cls_score = mx.sym.Convolution(name='rpn_cls_score', data=rpn_relu1, 
            num_filter=2*num_anchors, pad=(0,0), kernel=(1,1), stride=(1,1), no_bias=False, cudnn_off=False)
    rpn_bbox_pred = mx.sym.Convolution(name='rpn_bbox_pred', data=rpn_relu1, 
