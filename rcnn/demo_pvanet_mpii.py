...
from rcnn.symbol import get_pvanet_mpii_test
...
config.NUM_CLASSES = 2 #len(CLASSES)
...
config.HAS_PART = True
config.PART_GRID_HW = (6, 3)
...
def demo_net(predictor, image_name, vis=False):
    ...
    scores, boxes, head_boxes, joints, data_dict = im_detect(predictor, data_batch, data_names, im_scale)
    ...
    if vis:
        vis_all_detection(data_dict['data'].asnumpy(), boxes_this_image, CLASSES, im_scale, head_boxes, joints)
    else:
...
def main():
    ...
    if args.cpu:
        ...
        ctx = mx.gpu(args.gpu)
    symbol = get_pvanet_mpii_test(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS)
